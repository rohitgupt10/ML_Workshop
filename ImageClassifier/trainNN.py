...
def train(model: torch.nn.Module, 
    ...
    device: A target device to compute on (e.g. "cuda" or "cpu").
    save_path: Path to save the best model's weights (default is "best_model.pth").
    ...
    for epoch in tqdm(range(epochs)):
        ...
        if test_acc > best_test_acc:
            ...
            print(f"[INFO] New best model found! Saving model...")

    # After training, load the best weights into the model
    model.load_state_dict(best_model_wts)

    # Save the best model's weights to the specified path
    torch.save(best_model_wts, save_path)
    print(f"[INFO] Best model saved to {save_path}")

    # Return the results for all epochs
    return results
